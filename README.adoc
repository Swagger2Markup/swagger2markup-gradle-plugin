...
The configuration above would restult in call to Swagger2Markup:
...
ext {
    generatedSwagger = file("${buildDir}/swagger")
    generatedAsciiDoc = file("${buildDir}/asciidoc")
}

test {
    systemProperty 'io.springfox.staticdocs.outputDir', generatedSwagger
    outputs.dir generatedSwagger
}

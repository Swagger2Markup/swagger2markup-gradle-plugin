...
The Swagger2Markup Gradle Plugin adds a task which converts a Swagger JSON or YAML file into several *AsciiDoc* or *GitHub Flavored Markdown* documents which can be combined with hand-written documentation. The plugin supports the Swagger 1.2 and 2.0 specification. Internally it uses https://github.com/RobWin/swagger2markup[Swagger2Markup] which uses the _official_ https://github.com/swagger-api/swagger-parser[swagger-parser] and my https://github.com/RobWin/markup-document-builder[markup-document-builder].
